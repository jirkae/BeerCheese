  ...
  return mockSuppliers.map((supplier) => {
    return (
      <tr key={supplier.id}>
        ...
        <td>
          <Button onClick={() => openModal({name:'editSupplierAdmin',data:supplier})}>
...
const AdminSuppliersPage = (props) => (
  <div>
    ...
    <Container>
      ...
      <Button onClick={() => props.openModal({name:'newSupplierAdmin',data:null})}>{localizedTexts.AdminSuppliersPage.btnAddSupplier}</Button>
    </Container>
  </div>
)
